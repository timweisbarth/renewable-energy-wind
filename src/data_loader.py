...
def load_kelmarsh_data(from_raw):
    ...
    if from_raw:
        kwf2_paths = [
                "../data/raw/uk/Kelmarsh_SCADA_2016_3082/Turbine_Data_Kelmarsh_2_2016-01-03_-_2017-01-01_229.csv",
                "../data/raw/uk/Kelmarsh_SCADA_2017_3083/Turbine_Data_Kelmarsh_2_2017-01-01_-_2018-01-01_229.csv",
                "../data/raw/uk/Kelmarsh_SCADA_2018_3084/Turbine_Data_Kelmarsh_2_2018-01-01_-_2019-01-01_229.csv",
                "../data/raw/uk/Kelmarsh_SCADA_2019_3085/Turbine_Data_Kelmarsh_2_2019-01-01_-_2020-01-01_229.csv",
                "../data/raw/uk/Kelmarsh_SCADA_2020_3086/Turbine_Data_Kelmarsh_2_2020-01-01_-_2021-01-01_229.csv",
                "../data/raw/uk/Kelmarsh_SCADA_2021_3087/Turbine_Data_Kelmarsh_2_2021-01-01_-_2021-07-01_229.csv"
            ]
...
def flatten_multiindex(df,features_with_height_dim,features_with_range_dim):
    ...
    if features_with_height_dim:
        ...
        #Rename columns
        df_height1.columns = [column_name + "1" for column_name in features_with_height_dim]
        df_height2.columns = [column_name + "2" for column_name in features_with_height_dim]

        df_height = pd.merge(df_height1,df_height2, left_index=True, right_index=True)
    ...
    if features_with_range_dim:
        ...
        # Rename columns
        df_range1.columns = [column_name + "1" for column_name in features_with_range_dim]
        df_range2.columns = [column_name + "2" for column_name in features_with_range_dim]

        df_range = pd.merge(df_range1,df_range2, left_index=True, right_index=True)
    ...
    if features_with_height_dim:
        df_combined = pd.merge(df_rest, df_height, left_index=True, right_index=True).drop(columns=["Height"])    
    if features_with_range_dim:
        df_combined = pd.merge(df_combined, df_range, left_index=True, right_index=True).drop(columns=["Range","range","Turbine"])
...
def load_ueps_data(from_raw):
    ...
    if from_raw:
        ...
        ueps = flatten_multiindex(ueps, features_with_height_dim, features_with_range_dim)
